...
# --- KONFIGURASI WEB SERVER (Agar Render tidak 'Failed') ---
app = Flask(__name__)
@app.route('/')
def home():
...
# Jalankan web server di background agar Render mendeteksi port
t = Thread(target=run_web)
t.start()

# --- KONFIGURASI BOT ---
# Pastikan API_TOKEN diisi di Environment Variables di Render
API_TOKEN = os.environ.get('API_TOKEN', '8672273549:AAHz1TJiqkKXwTy6bUjrthI5--zyIlREJ8I')
bot = telebot.TeleBot(API_TOKEN)

# --- DAFTAR GAME (Sama seperti sebelumnya) ---
...
def handle_pola(m):
    ...
    if game in daftar_pgsoft:
        bot.send_message(m.chat.id, buat_pola_pgsoft(game), parse_mode='HTML')
    elif game in daftar_pragmatic:
        bot.send_message(m.chat.id, buat_pola_pragmatic(game), parse_mode='HTML')
    else:
